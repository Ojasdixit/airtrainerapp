...
export default function ReviewsScreen({ navigation }: any) {
    ...
    const renderStars = (rating: number) => (
        <View style={styles.starsRow}>
            {[1, 2, 3, 4, 5].map((star) => (
                <Ionicons key={star} name={star <= rating ? 'star' : 'star-outline'} size={16} color={star <= rating ? '#ffab00' : Colors.textTertiary} />
            ))}
        </View>
    );
...
const styles = StyleSheet.create({
    ...
    summaryCard: { alignItems: 'center', padding: Spacing.xl, marginHorizontal: Spacing.xxl, marginTop: Spacing.lg, backgroundColor: Colors.card, borderRadius: BorderRadius.lg, borderWidth: 1, borderColor: Colors.border, gap: Spacing.xs },
    summaryRating: { fontSize: 40, fontWeight: FontWeight.bold, color: Colors.text },
    ...
    reviewCard: { backgroundColor: Colors.card, borderRadius: BorderRadius.lg, padding: Spacing.lg, marginBottom: Spacing.md, borderWidth: 1, borderColor: Colors.border },
